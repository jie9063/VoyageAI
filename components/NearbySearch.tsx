...
import { MapPin, Search, Navigation, Clock, Star, Utensils, Camera, ShoppingBag, Loader2, History, Trash2, ArrowRight } from 'lucide-react';
...
export const NearbySearch: React.FC<NearbySearchProps> = () => {
  ...
  const handleSearch = async (loc: string) => {
    ...
    try {
      const places = await searchNearbyPlaces(loc);
      ...
      <div className="bg-white p-6 rounded-2xl shadow-lg border border-slate-100 mb-8 relative z-20">
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <button
            ...
            disabled={isLoading}
            className="flex items-center justify-center gap-2 px-6 py-3 bg-slate-100 text-slate-700 font-medium rounded-xl hover:bg-slate-200 transition-colors disabled:opacity-50"
            title="使用目前位置"
          >
